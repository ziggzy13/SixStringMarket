...
public class GuitarDAO {
	...
	public List<Guitar> searchGuitars(Map<String, Object> criteria) {
		...
		if (criteria.containsKey("keyword")) {
			...
			// Подобрено търсене по ключова дума
			if (keyword != null && !keyword.trim().isEmpty()) {
				...
				for (int i = 0; i < searchTerms.length; i++) {
					...
					// Търсене във всички текстови полета
					sqlBuilder.append("(title LIKE ? OR brand LIKE ? OR model LIKE ? OR description LIKE ?)");
				}
			...
			if (criteria.containsKey("keyword")) {
				...
				if (keyword != null && !keyword.trim().isEmpty()) {
					...
					for (String term : searchTerms) {
						// За всеки терм добавяме 4 параметъра (за всяко търсено поле)
